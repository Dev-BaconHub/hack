...
local BaconHubBlur = {}
BaconHubBlur.__index = BaconHubBlur

function BaconHubBlur.new(object)
...
function BaconHubBlur:_CreateDepthOfField()
    local existingDOF = lg:FindFirstChild("BaconHubBlur")
    if existingDOF then
        existingDOF:Destroy()
    end
    local existingBlur = lg:FindFirstChild("BaconHubBlurEffect")
...
function BaconHubBlur:_CreateFolder()
    local existingFolder = workspace.CurrentCamera:FindFirstChild("BaconHubBlur")
...
function BaconHubBlur:_CreateRoot()
...
function BaconHubBlur:_CreateFrame()
...
function BaconHubBlur:_Render(distance)
...
function BaconHubBlur:_Initialize()
...
function BaconHubBlur:SetEnabled(enabled)
...
function BaconHubBlur:Destroy()
    if self._folder then
        self._folder:Destroy()
    end
    local dof = lg:FindFirstChild("BaconHubBlur")
    if dof then
        dof:Destroy()
    end
    local blur = lg:FindFirstChild("BaconHubBlurEffect")
    if blur then
        blur:Destroy()
    end
end
...
local function CreateNotificationContainer(screenGui)
    ...
    NotificationContainer = CreateInstance("Frame", {
        ...
        Size = UDim2.new(0, 220, 1, -30),
        Parent = screenGui
    })
    CreateListLayout(NotificationContainer, 10, Enum.SortOrder.LayoutOrder, Enum.FillDirection.Vertical)
    NotificationContainer.UIListLayout.VerticalAlignment = Enum.VerticalAlignment.Bottom
    return NotificationContainer
end

function Library.new(title, configFolder)
    local self = setmetatable({}, Library)
    self.title = title or "BaconHub"
    self.tabs = {}
    self.currentTab = nil
    self.minimized = false
    self._baconHubBlur = nil
    self._keybinds = {}
...
function Library:Toggle()
    ...
    if self._baconHubBlur then
        self._baconHubBlur:SetEnabled(self._visible)
    end
    
    if self._updateFloatingToggle then
        self._updateFloatingToggle()
    end
end
...
function Library:_SetupMobileSupport()
    ...
    Connections["floatingtoggle_render"] = rs.RenderStepped:Connect(function()
        if holdStarted > 0 and (tick() - holdStarted >= holdThreshold) and not dragging then
            dragging = true
        end
    ...
    local function UpdateButtonState()
        local size = self._visible and 40 or 30
        CreateTween(icon, {Size = UDim2.new(0, size, 0, size)}, animationspeed.Fast)
        CreateTween(mobileButton, {BackgroundTransparency = self._visible and 0 or 0.25}, animationspeed.Fast)
    end

    self._mobileToggle = mobileButton
    self._mobileToggleIcon = icon
    self._updateFloatingToggle = UpdateButtonState
    UpdateButtonState()
end
...
function Library:_CreateMainv0rtexd()
    ...
    self.logoIcon = CreateInstance("ImageLabel", {
        Name = "Logo",
        Image = "rbxassetid://93449356170127",
        ImageColor3 = c.Text,
        BackgroundTransparency = 1,
    ...
    self._baconHubBlur = BaconHubBlur.new(self.container)
end
...
function Library:_ToggleMinimize()
    ...
    if self.minimized then
        if self._baconHubBlur then
            self._baconHubBlur:SetEnabled(false)
        end
        ...
        if self._baconHubBlur then
            self._baconHubBlur:SetEnabled(true)
        end
...
function Library:CreateTab(name, icon)
    ...
    clickBtn.MouseButton1Click:Connect(function()
        Library._SelectTab(lib, tab, tabBtn, tabStroke, iconLabel, tabText, textGradient)
    end)
    ...
    function tabMethods:CreateKeybind(config)
        return Library._CreateKeybind(self, config, lib)
    end
...
function Library._CreateToggle(tab, config)
    ...
    local nameLabel = CreateInstance("TextLabel", {
        ...
        TextXAlignment = Enum.TextXAlignment.Left,
        BackgroundTransparency = 1,
        Position = description and UDim2.new(0, 10, 0, 8) or UDim2.new(0, 10, 0.5, -10),
        TextSize = textsize.Normal,
        Size = UDim2.new(0, 200, 0, 20),
        Parent = frame
    })
    ...
    if description then
        ...
        local function UpdateFrameHeight()
            frame.Size = UDim2.new(1, 0, 0, 30 + descLabel.AbsoluteSize.Y + 10)
        end
        descLabel:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateFrameHeight)
